    ...
    Runnable checkForNewData = new Runnable()
    {
        ...
        public void run()
        {
            if(individualArray != null)
            {
                ...
                try
                {
                    String lat = Double.toString(retrievedData.lat);
                    String lon = Double.toString(retrievedData.longitude);
                    if(individualArray.getJSONObject(0).getBoolean("protected") && !g.GetAdmin())
                    {
